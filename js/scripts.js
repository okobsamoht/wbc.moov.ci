...
var GetChaturl = "http://webcoom.net/mobile/moov_chat/getChatData.php";
var SendChaturl = "http://webcoom.net/mobile/moov_chat/sendChatData.php";
...
function insertNewContent(liName,liText) {
	...
	oLi.appendChild(oText);
	//insertO.insertBefore(oLi, insertO.firstChild);
	$('#outputList').append('<li><span class="name">'+liName+'</span>'+liText+'</li>')
...
function checkName() {
	...
	if (currentName.value == '') {
		currentName.value = numero; //'guest_'+ Math.floor(Math.random() * 10000);
	}
	if (numero == '') {
		prompt('Veuiller entrer votre numero moov');
	}
}
